...
public class LascaBoard {
	private LascaBoardSpace []
			[] board = new LascaBoardSpace [7]
			[7];
			 
			private LascaBoardSpace lastPlayer = LascaBoardSpace.Player1;
			...
			public LascaBoardSpace get (int row, int column)
			...
			public LascaBoardPosition findPosition(LascaBoardSpace what)
			 
			{
			 
			for (int row = 0; row < board.length; row++)
			 
			for (int column = 0; column < board[0]
			.length; column++)
			 
			if (board[row]
			[column] == what)
			 
			return new LascaBoardPosition(row, column);;
			...
			public void movePlayer (LascaBoardSpace which, LascaBoardPosition newPosition)
			...
			if (which != LascaBoardSpace.Player1 && which != LascaBoardSpace.Player2)
			...
			LascaBoardPosition position = findPosition(which);
			...
			if (board[newPosition.getRow()
			]
			[newPosition.getColumn()
			] != LascaBoardSpace.Available)
			 
			return;
			 
			 
			if (Math.abs(position.getRow() - newPosition.getRow()
			) > 1 || Math.abs(position.getColumn() - newPosition.getColumn()
			) > 1)
			...
			board[newPosition.getRow()]
			[newPosition.getColumn()] = which;
			 
			}
			 
			 
			public LascaBoardSpace checkWinner ()
			...
			LascaBoardPosition position1 = findPosition(LascaBoardSpace.Player1);
			 
			LascaBoardPosition position2 = findPosition(LascaBoardSpace.Player2);
			...
			return LascaBoardSpace.Player2;
			 
			 
			if (player2Isolated && ! player1Isolated)
			 
			return LascaBoardSpace.Player1;
			...
			return LascaBoardSpace.Available;
			...
			board = new LascaBoardSpace[height]
			[width];
			...
			board[row]
			[column] = LascaBoardSpace.Available;
			...
			board[0]
			[startColumn] = LascaBoardSpace.Player1;
			 
			board[height-1]
			[startColumn] = LascaBoardSpace.Player2;
			 
			}
			 
			 
			private boolean positionIsIsolated (LascaBoardPosition position)
